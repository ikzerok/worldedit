                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    for part in parts {
                        if let Some(target) = part.target {
                            if ui.link(&part.text).on_hover_text("阅读关联对象").clicked() {
                                self.open_reading(target);
                            }
                        } else {
                            ui.add(egui::Label::new(RichText::new(&part.text).size(16.0)).wrap());
                        }
                    }
                });
            });
                ...
                ui.push_id((&target.kind, &target.id), |ui| {
                    ...
                    ui.horizontal_wrapped(|ui| {
                        for name in &names {
                                ...
                                .clicked()
                            {
                                ...
                                self.commit("别名已移除", |p| {
                                    p.set_aliases(&target, &remaining)
                                });
                            }
                        }
                    });
                    ...
                    if let Some(properties) = properties {
                        for (key, value) in properties {
                            ...
                            ui.add(
                                egui::Label::new(if text.is_empty() {
                                    "尚未填写"
                                } else {
                                    &text
                                })
                    ...
                    for state in catalog.states.values().filter(|s| {
                        s.target == target || (target.kind == "state" && target.id == s.id)
                    }) {
